...
local function ensurePanelLayout(frame)
    ...
    for slot = 1, 6 do
        ...
        cover.Paint = function(self, w, h)
            local outage = frame._state and frame._state.powerOutage == true
            draw.RoundedBox(10, 0, 0, w, h, outage and Color(28, 30, 34, 250) or Color(78, 82, 90, 245))
            surface.SetDrawColor(outage and Color(180, 72, 72, 255) or Color(140, 146, 154, 255))
            surface.DrawOutlinedRect(0, 0, w, h, 2)
            for y = 18, h - 18, 20 do
                surface.SetDrawColor(outage and Color(80, 28, 28, 110) or Color(98, 104, 112, 90))
                surface.DrawLine(14, y, w - 14, y)
            end
            draw.SimpleText(outage and "POWER OUTAGE" or "MODULE LOCKED", "KTNE_Title", w / 2, h / 2 - 12, Color(230, 235, 240), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
            draw.SimpleText(outage and "Restore circuits in datapad" or "Identify bomb make in manual", "KTNE_Body", w / 2, h / 2 + 18, Color(200, 208, 214), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        end
...
local function buildBombIdentifyPages()
    local pages = {
        {
            ...
            lines = {
                ...
                "For testing, the current bomb make is locked to DETPACK.",
            }
        }
    }
...
local function buildManualPages(state)
    ...
    local pages = buildBombIdentifyPages()
...
local function ensureManualLayout(frame)
    ...
    frame.ContentHolder:Clear()
    local outer = vgui.Create("DPanel", frame.ContentHolder)
    outer:Dock(FILL)
...
local function updateIdentificationVisuals(frame)
    ...
    if frame.ModuleCovers then
        for slot, cover in pairs(frame.ModuleCovers) do
            ...
            local covered = ((not identified) or (state.powerOutage == true)) and activeId ~= nil and slot >= 1 and slot <= 6
            if IsValid(cover) then cover:SetVisible(covered) end
        end
    end
end
...
local function updateHeader(frame)
    ...
    elseif frame._role == "solo" then
        local class = IsValid(frame._ent) and frame._ent:GetClass() or ""
        local titles = {
            sent_ktne_detpack_solo = "Detpack",
            sent_ktne_emp_solo = "EMP Bomb",
            sent_ktne_incendiary_solo = "Incendiary Bomb",
        }
        title = titles[class] or "Solo Bomb"
...
local function ensureDatapadLayout(frame)
    ...
    canvas.Paint = function(_, w, h)
        ...
        local activeModule = seismic.active == true or pkb.active == true
    ...
    pkbGrid.Think = function(self)
        ...
        if IsValid(frame.DatapadPillars) then frame.DatapadPillars:SetVisible(pkb.active ~= true) end
...
frame.DatapadHoldButton = nil
end

local function ensureSoloLayout(frame)
    if IsValid(frame.SoloTabs) then return end
    frame.ContentHolder:Clear()
    ...
    frame._panelPage = vgui.Create("EditablePanel", stack)
    frame._panelPage:Dock(FILL)
    frame._panelPage.Paint = function(_, w, h)
        draw.RoundedBox(8, 0, 0, w, h, Color(4, 14, 22, 252))
    end

    frame._manualPage = vgui.Create("EditablePanel", stack)
    frame._manualPage:Dock(FILL)
    frame._manualPage:SetVisible(false)
    frame._manualPage.Paint = function(_, w, h)
        draw.RoundedBox(8, 0, 0, w, h, Color(4, 14, 22, 252))
    end
    ...
    local function setSoloView(view)
        if view ~= "datapad" and IsValid(frame.DatapadHoldButton) and frame.DatapadHoldButton._holding then
            frame.DatapadHoldButton._holding = false
            sendAction(frame._ent, "radio_hold", {holding = false}, 0.05, "release")
        end
        frame._soloView = view
        if IsValid(frame._panelPage) then frame._panelPage:SetVisible(view == "panel") end
        if IsValid(frame._manualPage) then frame._manualPage:SetVisible(view == "manual") end
        if IsValid(frame._datapadPage) then frame._datapadPage:SetVisible(view == "datapad") end
        if IsValid(frame._panelTab) then frame._panelTab:InvalidateLayout(true) end
    ...
    frame._setSoloView = setSoloView

    local function makeSoloTab(label, viewName)
        ...
        btn:SetWide(math.max(90, tw + 30))
        btn._displayText = label
        btn._viewName = viewName
        btn.Paint = function(self, w, h)
            local active = frame._soloView == self._viewName
            local bg = active and Color(16, 58, 84, 245) or Color(8, 30, 44, 220)
            local edge = active and THEME.line or THEME.lineSoft
            local textCol = THEME.text
            if self._viewName == "datapad" then
                local seismic = (frame._state and frame._state.seismicControl) or {active = false, phase = "idle", timer = 0}
                local pkb = (frame._state and frame._state.pkbControl) or {active = false, phase = "idle", timer = 0}
                if pkb.active == true and pkb.phase == "outage" then
                    local blinkOn = math.floor(CurTime() * 4) % 2 == 0
                    bg = blinkOn and Color(108, 18, 18, 245) or Color(44, 12, 12, 220)
                    edge = Color(230, 90, 90)
                    textCol = Color(255, 212, 212)
                elseif seismic.active == true and seismic.phase == "alarm" then
                    local blinkOn = math.floor(CurTime() * 4) % 2 == 0
                    bg = blinkOn and Color(108, 18, 18, 245) or Color(44, 12, 12, 220)
                    edge = Color(230, 90, 90)
                    textCol = Color(255, 212, 212)
        ...
        btn.DoClick = function()
            setSoloView(viewName)
        end
        return btn
    end

    frame._soloView = "panel"
    frame._panelTab = makeSoloTab("Panel", "panel")
    frame._manualTab = makeSoloTab("Manual", "manual")
    frame._datapadTab = makeSoloTab("Datapad Access Terminal", "datapad")
    ensureDatapadLayout(frame)
    setSoloView("panel")
end


local function refreshSoloDatapadTab(frame)
    ...
    if not show and frame._soloView == "datapad" and frame._setSoloView then
        frame._setSoloView("panel")
    end
end
...
if role == "solo" then
    ensureSoloLayout(frame)
    refreshSoloDatapadTab(frame)

    local old = frame.ContentHolder
    frame.ContentHolder = frame._panelPage
    updatePanel(frame)

    frame.ContentHolder = frame._manualPage
    updateManual(frame)
    frame.ContentHolder = old
...
net.Receive("ktne_open_ui_solo", function()
...
net.Receive("ktne_sync_state_solo", function()
    local ent = net.ReadEntity()
    local role = net.ReadString()
    local state = net.ReadTable() or {}
...
net.Receive("ktne_close_ui_solo", function()
    local ent = net.ReadEntity()
    hiddenFrames[ent] = nil
    closeFrameForBomb(ent, false)
end)
...
function ENT:Draw()
    ...
    cam.Start3D2D(pos, ang, 0.12)
        ...
        draw.SimpleText("INCENDIARY BOMB", "KTNE_Title", 0, -46, THEME.title, TEXT_ALIGN_CENTER)
        local line1 = self:GetGameActive() and ("Time: " .. self:GetTimeRemaining() .. "s") or "Use to start Incendiary"
        local line2 = "Make: UNIDENTIFIED  |  Serial: ????"
        if self:GetPanelPlySID() ~= "" then
            line2 = "Player: " .. (((self:GetPanelPlySID() ~= "") and "1/1" or "0/1"))
        end
