...
util.AddNetworkString("ktne_open_ui_solo")
util.AddNetworkString("ktne_sync_state_solo")
util.AddNetworkString("ktne_action_solo")
util.AddNetworkString("ktne_close_ui_solo")
...
function ENT:Initialize()
    ...
    if self.UseTriggerBounds then self:UseTriggerBounds(true, 24) end
...
function ENT:GenerateModules()
    ...
    self.ModuleSlots[5] = "reactor"
    self.Modules.reactor = generated.reactor
    self.ModuleSlots[6] = "drill"
    self.Modules.drill = buildDrillModule(self.Serial, self.Modules.tamper ~= nil)
    ...
    self.ManualData = {
        ...
        overview = {
            ...
            lines = {
                ...
                "Four live modules were selected from the current random module pool.",
                "The first four bomb slots are randomized when the round starts. Slot 5 is reserved for test modules.",
                "Solve all four active modules before the timer reaches zero.",
...
function ENT:Use(activator)
    ...
    if not role then
        activator:ChatPrint("This solo test bomb is already in use by another player.")
        return
    end
...
function ENT:GetClientStateFor(role, includeStatic, syncFlags, ply)
    ...
    if includeDynamic then
        ...
        state.radioactiveSeal = deepCopy(self.RadioSeal or {progress = 0, holding = false})
...
function ENT:StartGame()
    ...
    self:NotifyPlayers("Solo test bomb started. Use the tabs to switch between the panel, manual, and any active datapad systems. Current timer: " .. formatChatTimeRemaining(startTime) .. ".")
...
function ENT:UpdateRadioSealTimer()
    -- Radioactive seal now updates directly from hold pulses.
end
...
function ENT:HandlePanelAction(ply, action, data)
    ...
    elseif action == "chat_ack" then
        if ply == self.PanelPlayer or ply == self.ManualPlayer then
...
net.Receive("ktne_action_solo", function(len, ply)
    ...
    if class ~= "sent_ktne_bomb_solo" and class ~= "sent_ktne_detpack_solo" and class ~= "sent_ktne_emp_solo" and class ~= "sent_ktne_incendiary_solo" then return end
...
function ENT:SyncState(force, syncFlags)
    ...
    for _, ply in ipairs(targets) do
        local role = (ply == self.PanelPlayer and ply == self.ManualPlayer) and "solo" or ((ply == self.PanelPlayer) and "panel" or "manual")
